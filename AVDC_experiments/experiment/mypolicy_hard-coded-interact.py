...
class MyPolicy_CL_rgbd(Policy):
    def __init__(self, env, task, camera, video_model, flow_model, resolution=(640, 480), plan_timeout=20, max_replans=5, log=False):
        ...
        self.tasks = ['grasp door', 'door open']
        subgoals = []
        ...
        if self.is_traj:
            subgoals = self.process_flow(subgoals)
        subgoals_np = np.array(subgoals)
        self.mode = 'push'
    ...
    def calculate_grasp(self, name=None):
        ...
        pts_3d = torch.median(torch.from_numpy(pts_3d), dim=0)[0].detach().cpu().numpy() + np.array([0, 0, 0.0])
        self.grasp = pts_3d
        self.grasp_2d = torch.median(pts_2d, dim=0)[0]
    ...
    def calculate_traj(self):
        ...
        self.pred_image_depth_segms = np.concatenate([images, depths, segms], axis=1).transpose(0, 2, 3, 1)

        _, _, _, flow, flow_b = pred_flow_frame(self.flow_model, images)
    ...
    def _desired_pos(self, o_d):
        ...
        if self.replan_countdown <= 0 and self.replans > 0:
            ...
            if self.is_traj:
                return self.process_flow(self.subgoals)
            return self.subgoals[0]

        if self.is_traj:
            subgoal = self.process_flow(self.subgoals[0])
        else:
            subgoal = self.subgoals
        ...
        if np.linalg.norm((pos_curr - self.subgoals[0]) ) > move_precision:
            ...
            if len(self.subgoals) == 1:
                ...
                if len(self.tasks) > 1:
                    ...
                    if self.is_traj:
                        subgoal = self.process_flow(self.subgoals)

                        import ipdb;ipdb.set_trace()
                        import cv2
                        pred_image_depth_segm = self.pred_image_depth_segms[0]
                        rgb2 = pred_image_depth_segm[..., :3]
                        pos2 = self.subgoal_2d
                        rgb1 = self.previous_image
                        pos1 = self.pos_curr_2d
                        rgb1 = cv2.circle(rgb1, [int(pos1[0]), int(pos1[2])], radius=10, color=[255, 0, 0], thickness=-1)
                        rgb1 = cv2.circle(rgb2, [int(pos2[0]), int(pos2[2])], radius=10, color=[255, 0, 0], thickness=-1)
                        cv2.imwrite('debug1.png', rgb1)
                        cv2.imwrite('debug2.png', rgb2)
                        import ipdb;ipdb.set_trace()
                        return subgoal
                    return self.subgoals[0]
                else:
    ...
    def process_flow(self, subgoals):
        flow = subgoals[0]
        subgoal_2d = self.pos_curr_2d + flow[int(self.pos_curr_2d[1]), int(self.pos_curr_2d[0])]
        pred_image_depth_segm = self.pred_image_depth_segms[0]
        depth = pred_image_depth_segm[..., 3]
        cmat = get_cmat(self.env, self.camera, resolution=self.resolution)
        subgoal_3d = to_3d(subgoal_2d, depth, cmat)
        ## TODO:
